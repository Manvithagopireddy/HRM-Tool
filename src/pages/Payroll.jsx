...
import { DollarSign, Download, X, Printer, CheckCircle, Clock, FileSpreadsheet } from 'lucide-react';
import { useHRM } from '../context/HRMContext';
import { useToast } from '../context/ToastContext';
import { exportToCSV, exportToExcel, exportPayslipPDF, PAYROLL_EXCEL_COLS, printElement } from '../utils/exportUtils';
...
const buildPayroll = (employees) => employees.map(e => {
  ...
  return {
    ...
    net: gross - tax - insurance - retirement,
...
const PayslipModal = ({ record, onClose }) => {
  ...
  const items = [
    ...
    { label: 'Retirement Fund (3%)',   earn: 0, deduct: record.retirement },
  ];
  return (
      ...
      <div className="modal modal-lg slide-up">
        <div className="modal-header">
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button className="btn btn-sm btn-primary" onClick={() => exportPayslipPDF(record)}>
              <Download size={14}/> Download PDF
            </button>
            <button className="btn btn-sm btn-secondary" onClick={() => printElement('payslip-content')}>
          ...
          <div id="payslip-content" className="payslip">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px', padding: '16px', background: '#f9fafb', borderRadius: '8px' }}>
              {[{ k: 'Employee Name', v: record.name }, { k: 'Employee ID', v: record.empId }, { k: 'Job Title', v: record.role }, { k: 'Department', v: record.department }].map(r => (
                <div key={r.k}>
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase' }}>{r.k}</div>
                  <div style={{ fontWeight: 600, color: '#111827', marginTop: '2px' }}>{r.v}</div>
                </div>
              ))}
            </div>
            <table>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.label}>
                    <td>{item.label}</td>
                    <td style={{ textAlign: 'right' }}>{item.earn   > 0 ? `$${item.earn.toLocaleString()}`   : '—'}</td>
                    <td style={{ textAlign: 'right' }}>{item.deduct > 0 ? `$${item.deduct.toLocaleString()}` : '—'}</td>
                  </tr>
                ))}
                <tr className="payslip-total">
                  <td>Net Pay</td>
                  <td style={{ textAlign: 'right' }} colSpan={2}>${record.net.toLocaleString()}</td>
                </tr>
              </tbody>
            </table>
...
const Payroll = () => {
  ...
  const totalRetirement = payrollData.reduce((a, e) => a + e.retirement, 0);

  const handleExcelExport = () => {
    exportToExcel([
      { name: 'April 2026 Payroll', data: payrollData, columns: PAYROLL_EXCEL_COLS },
      {
        name: 'Summary',
        data: [
          ['Metric', 'Value'],
          ...
          ['Retirement Fund', `$${totalRetirement.toLocaleString()}`],
          ['Employees Paid',  payrollData.filter(e => e.status === 'Processed').length],
          ['Pay Period',      'April 2026'],
        ],
      },
    ], 'peoplecore_payroll');
    toast('Payroll exported to Excel (.xlsx)!', 'success');
  };
  ...
  const barData = {
    ...
    datasets: [{ label: 'Total Payroll', data: deptPayroll.map(d => d.total), backgroundColor: ['rgba(255,255,255,0.75)', 'rgba(255,255,255,0.55)', 'rgba(255,255,255,0.38)', 'rgba(255,255,255,0.25)', 'rgba(255,255,255,0.15)'], borderRadius: 4 }],
  };
  const donutData = {
    labels: ['Net Pay', 'Tax', 'Insurance', 'Retirement'],
    datasets: [{ data: [totalNet, totalTax, payrollData.reduce((a, e) => a + e.insurance, 0), totalRetirement], backgroundColor: ['#f9fafb', '#9ca3af', '#6b7280', '#374151'], borderWidth: 0, hoverOffset: 4 }],
  };
    ...
    <div className="fade-in">
      <div className="page-header">
        ...
        <div className="page-header-actions">
          ...
          <button className="btn btn-primary" onClick={() => toast('Payroll processed for April 2026! 🎉', 'success')}>
            <CheckCircle size={16}/> Run Payroll
          </button>
        </div>
      </div>
      ...
      <div className="kpi-grid">
        {[
          ...
          { label: 'Total Net Pay', value: `$${(totalNet/1000).toFixed(1)}k`,    sub: 'after all deductions',             color: 'success' },
          { label: 'Tax Withheld',  value: `$${(totalTax/1000).toFixed(1)}k`,    sub: 'federal income tax',               color: 'danger' },
          { label: 'Retirement',    value: `$${(totalRetirement/1000).toFixed(1)}k`, sub: '3% contribution fund',         color: 'warning' },
        ].map(k => (
      ...
      <div className="table-wrapper">
        ...
        <table>
          <thead>
            <tr><th>Employee</th><th>Department</th><th>Gross</th><th>Tax</th><th>Insurance</th><th>Retirement</th><th>Net Pay</th><th>Status</th><th>Action</th></tr>
          </thead>
          <tbody>
            {filtered.map(e => (
              <tr key={e.empId}>
                ...
                <td style={{ color: 'var(--info)' }}>-${e.retirement.toLocaleString()}</td>
